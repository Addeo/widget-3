...
fetch('https://api.leads.convolo.ai/api/v1/support/check-ip/my')
    ...
    response.json().then(responseParse => {
        ...
        if (uaeWidgetsCountryArray.includes(countryCode.toUpperCase())) {
            (function f() { var widget_key = uaeWidgetKeyLeadsPage; window.leadCM = { widget_key: widget_key, }; var em = document.createElement('script'); em.type = 'text/javascript'; em.async = true; em.src = 'https://app.leadconnect.cc/js/icallback.js?v=' + Math.random() + '&key=' + widget_key + '&uri=' + encodeURIComponent(window.location.href); var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(em, s); })();
        } else {
